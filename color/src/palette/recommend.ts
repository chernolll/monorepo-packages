...
import { getDeltaE, getHsl, isValidColor, transformHslToHex } from '../shared/colord';

import { colorPalettes } from '../constant/palette'

import type { ColorPalette, ColorPaletteFamily, ColorPaletteFamilyWithNearestPalette, ColorPaletteMatch, ColorPaletteNumber } from "../type";
...
function getRecommendedColorPaletteFamily(color: string) {
  if (!isValidColor(color)) {
    throw new Error('Invalid color, please check the color value');
  }
  ...
  const colorPaletteFamily: ColorPaletteFamily = {
    ...
    palettes: palettes.map(palette => {
      ...
      if (!isSame) {
        ...
        hexValue = transformHslToHex({ h: newH, s: newS, l });
      }

      return { hex: hexValue, number: palette.number }
    })
  };
...
function getNearestColorPaletteFamily(color: string, families: ColorPaletteFamily[]) {
  const familyWithConfig = families.map(family => {
    const palettes = family.palettes.map(palette => ({ ...palette, delta: getDeltaE(color, palette.hex) }));

    const nearestPalette = palettes.reduce((prev, curr) => (prev.delta < curr.delta ? prev : curr));

    return { ...family, palettes, nearestPalette, }
  });

  const nearestPaletteFamily = familyWithConfig.reduce((prev, curr) => {
    return prev.nearestPalette.delta < curr.nearestPalette.delta ? prev : curr;
  });
